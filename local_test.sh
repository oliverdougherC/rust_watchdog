...
PRESET_FILE="${WATCHDOG_REAL_PRESET_FILE:-"$ROOT_DIR/presets/AV1_MKV.json"}"
...
print_usage() {
  cat <<'USAGE'
Usage: ./local_test.sh [video_dir] [cargo-test-args...]

Examples:
  ./local_test.sh
  ./local_test.sh test_videos
  ./local_test.sh --test healthcheck_cli
  ./local_test.sh --video-dir test_videos --test healthcheck_cli
  ./local_test.sh --full-sim-once
